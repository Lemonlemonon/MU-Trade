...
<title>${siteName!""}|Logs management-${title!""}</title>
...
function delAll(url){
	$.confirm({
        title: 'Clear the entire log table',
        content: 'Data can not be restore, be careful!',
        buttons: {
            confirm: {
                text: 'Confirm',
                action: function(){
...
function deleteReq(ids,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('Logs removed!',function(){
					window.location.reload();
